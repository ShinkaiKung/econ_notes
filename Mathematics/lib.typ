...
#let template(
  ...
  show selector(heading.where(level: 1)): set heading(
    numbering: (..nums) => h1-prefix + " " + nums.pos().map(str).join(".") + ":",
  )
...
#let concept(name) = [
  #text(style: "italic", weight: "bold", fill: blue)[#name]
]

#let section(title) = text(fill: maroon)[#upper[*#title*]]
